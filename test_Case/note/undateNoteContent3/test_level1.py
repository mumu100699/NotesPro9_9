...
@class_case_log
class UpdateNotesContent1(unittest.TestCase):
    ...
    @parameterized.expand(('noteId', ))  # 所以执行的时候要从类执行，class
    def test02_must_key(self, key):
        ...
        get_userid_res = self.apiRe.note_post(self.get_userid_url, self.user_id, self.sid, body)
        self.assertEqual(500, get_userid_res.status_code)
        expect_output = {'errorCode': int, 'errorMsg': str}
        CheckTools().check_output(expect_output, get_userid_res.json())  # actual_output等于res.json()

    @parameterized.expand(('title',))  # 所以执行的时候要从类执行，class
    def test03_must_key(self, key):
        """上传/更新便签内容主流程，必填项校验title"""
        print(f'必填项校验的字段{key}')
        print('前置步骤：上传便签信息主体')
    ...
    @parameterized.expand(('localContentVersion',))  # 所以执行的时候要从类执行，class
    def test04_must_key(self, key):
        """上传/更新便签内容主流程，必填项校验localContentVersion"""
        print(f'必填项校验的字段{key}')
        print('前置步骤：上传便签信息主体')
