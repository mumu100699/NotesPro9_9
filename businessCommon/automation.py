...
def generate_notes(num, sid, user_id):
    ...
    for i in range(num):
        ...
        # expect_output = {'responseTime': int, 'infoVersion': int, 'infoUpdateTime': int}  # 期望结果：对齐文档。无需校验
        # CheckTools().check_output(expect_output, res.json())  # actual_output等于res.json()。无需校验
        ...
        # expect_output = {'responseTime': int, 'contentVersion': 1, 'contentUpdateTime': int} # 无需校验
        # CheckTools().check_output(expect_output, get_userid_res.json())  # actual_output等于res.json()。无需校验
